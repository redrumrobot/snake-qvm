...
vmCvar_t  g_NamesFile;
vmCvar_t  g_NamesFileSync;
vmCvar_t  g_blood;
...
vmCvar_t  g_adminReports;
vmCvar_t  g_adminTempBan;
vmCvar_t  g_adminMaxBan;
vmCvar_t  g_adminTempSpec;
vmCvar_t  g_Bubbles;
vmCvar_t  g_FireWorks;
vmCvar_t  g_noAbusableCMDs;
vmCvar_t  g_minNoRegisterLevel;
vmCvar_t  g_maxNoRegisterLevel;
vmCvar_t  g_NoRegisterLevels;
...
static cvarTable_t   gameCvarTable[ ] =
{
  ...
  { &g_NamesFile, "g_NamesFile", "names.txt", CVAR_ARCHIVE, 0, qfalse  },
  { &g_NamesFileSync, "g_NamesFileSync", "0", CVAR_ARCHIVE, 0, qfalse  },
  ...
  { &g_maxTKs, "g_maxTKs", "0", CVAR_ARCHIVE, 0, qfalse  },
  { &g_maxTKTime, "g_maxTKTime", "0", CVAR_ARCHIVE, 0, qfalse  },
  ...
  { &g_adminMaxBan, "g_adminMaxBan", "4w", CVAR_ARCHIVE, 0, qfalse  },
  { &g_Bubbles, "g_Bubbles", "1", CVAR_ARCHIVE, 0, qfalse  },
  { &g_FireWorks, "g_FireWorks", "1", CVAR_ARCHIVE, 0, qfalse  },
  { &g_noAbusableCMDs, "g_noAbusableCMDs", "1", CVAR_ARCHIVE, 0, qfalse  },
  { &g_minNoRegisterLevel, "g_minNoRegisterLevel", "2", CVAR_ARCHIVE, 0, qfalse  },
  { &g_maxNoRegisterLevel, "g_maxNoRegisterLevel", "6", CVAR_ARCHIVE, 0, qfalse  },
  { &g_NoRegisterLevels, "g_NoRegisterLevels", "0", CVAR_ARCHIVE, 0, qfalse  },
  { &g_adminReports, "g_adminReports", "0", CVAR_ARCHIVE, 0, qfalse  },
  ...
  { &g_reportLimit, "g_reportLimit", "2", CVAR_ARCHIVE, 0, qfalse  },
...
void G_InitGame( int levelTime, int randomSeed, int restart )
{
  ...
  if( g_NamesFile.string[ 0 ] )
  {
    if( g_NamesFileSync.integer )
      trap_FS_FOpenFile( g_NamesFile.string, &level.NamesFile, FS_APPEND_SYNC );
    else
      trap_FS_FOpenFile( g_NamesFile.string, &level.NamesFile, FS_APPEND );

    if( !level.NamesFile )
      G_Printf( "WARNING: Couldn't open Namesfile: %s\n", g_NamesFile.string );
  }
  else
    G_Printf( "Not logging Names to disk\n" );
...
void G_ShutdownGame( int restart )
{
  ...
  if( level.NamesFile )
  {
    trap_FS_FCloseFile( level.NamesFile );
  }
...
Print the names to a specific Namesfile.
=================
*/
void QDECL G_LogNames( const char *fmt, ... )
{
  va_list argptr;
  char    string[ 1024 ], decoloured[ 1024 ];

  va_start( argptr, fmt );
  vsprintf( string +0 , fmt,argptr );
  va_end( argptr );
  ...
  if( !level.NamesFile )
    ...
    G_DecolorString( string, decoloured );
    trap_FS_Write( decoloured, strlen( decoloured ), level.NamesFile );
}
...
void CheckExitRules( void )
{
        ...
        ( level.numLiveAlienClients == 0 ) ) )
  {
    ...
    trap_SendServerCommand( -1, "print \"^4Humans win^7\n\"");
    trap_SendServerCommand( -1, "cp \"^4Humans win^7\"");
             ...
             ( level.numLiveHumanClients == 0 ) ) )
  {
    ...
    trap_SendServerCommand( -1, "print \"^1Aliens win^7\n\"");
    trap_SendServerCommand( -1, "cp \"^1Aliens win^7\"");
